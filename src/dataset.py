...
class GPTDataset(Dataset):
    ...
    def __init__(
        ...
        if stride == None:
            self._length = (len(token_ids) - context_length -1) // context_length +1
        else:
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        input_ids = torch.tensor(self.token_ids[start: start + self.context_length]
                                 , dtype=torch.long)
        target_ids = torch.tensor(self.token_ids[start + 1: start + 1 + self.context_length]
                                  , dtype=torch.long)
...
def create_dataloader(
    ...
    dataset = GPTDataset(token_ids=token_ids, context_length=context_length, stride=stride)
